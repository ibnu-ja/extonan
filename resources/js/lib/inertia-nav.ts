...
export const inertiaNav: Attachment = (element) => {
    ...
    if (preserveScroll) {
        params.preserveScroll = preserveScroll === 'true';
    }

    const preserveState = element.getAttribute('data-preserve-state');

    if (preserveState) {
        params.preserveState = preserveState === 'true';
    }
    ...
    if (viewTransition) {
        params.viewTransition = viewTransition === 'true';
    }

    const only = element.getAttribute('data-only');

    if (only) {
        params.only = only.split(',').map((s) => s.trim());
    }

    const except = element.getAttribute('data-except');

    if (except) {
        params.except = except.split(',').map((s) => s.trim());
    }
