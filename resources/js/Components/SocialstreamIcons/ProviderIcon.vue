...
<template>
  <div class="text-gray-900">
    ...
    />
    <LinkedInIcon
      v-if="provider.id === 'linkedin' || provider.id === 'linkedin-openid'"
      :class="classes"
    />
    <SlackIcon
      v-if="provider.id === 'slack'"
      :class="classes"
    />
    <TwitterIcon
      v-if="provider.id === 'twitter' || provider.id === 'twitter-oauth-2'"
      :class="classes"
    />
  </div>
</template>
