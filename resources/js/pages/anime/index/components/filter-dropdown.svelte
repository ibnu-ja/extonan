...
<Popover.Root bind:open>
		...
		{#snippet child({ props })}
			<Button variant="outline" size="sm" {...props} class="gap-1">
				...
				{#if !singleSelect && activeCount > 0}
					<Badge variant="secondary" class="ml-1 size-5 rounded-full p-0 text-xs">{activeCount}</Badge>
				{/if}
			</Button>
		{/snippet}
	</Popover.Trigger>
	<Popover.Content class="w-64 p-0" align="start">
		<Command.Root>
			<Command.Input bind:value={search} placeholder={`Search ${label.toLowerCase()}...`} />
			<Command.List>
