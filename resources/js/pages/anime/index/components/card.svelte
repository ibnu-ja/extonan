<script lang="ts">
	...
	const displayTitle = $derived(item.title?.romaji ?? item.title?.english ?? item.title?.native ?? 'Untitled');
</script>

<Link href={item.link} class="group relative block overflow-hidden rounded-xl bg-muted">
	...
	<div class="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/80 to-transparent p-3 pt-8">
		<h3 class="text-sm font-medium leading-tight text-white line-clamp-2">{displayTitle}</h3>
	</div>
