...
<script lang="ts">
    ...
    const sortOptions = $derived((page.props.sortOptions as { key: string; value: string }[]) ?? []);

    let searchTimeout: ReturnType<typeof setTimeout> | undefined;

    function parseFilterState(): AnimeFilterState {
        const params = new URL(page.url, typeof window !== 'undefined' ? window.location.origin : 'http://localhost').searchParams;
    ...
    let sort = $state(new URL(page.url, typeof window !== 'undefined' ? window.location.origin : 'http://localhost').searchParams.get('sort') || 'title->romaji');
    const auth = $derived(page.props.auth as { permissions?: string[]; roles?: string[] });
    const canReadDrafts = $derived(auth?.permissions?.includes('post.read.self') ?? false);
    const filterIsPublished = $derived(new URL(page.url, typeof window !== 'undefined' ? window.location.origin : 'http://localhost').searchParams.get('filter[is_published]'));
    let publishFilter = $state<'published' | 'draft' | null>(
        filterIsPublished === 'true' ? 'published' : filterIsPublished === 'false' ? 'draft' : null,
    );
...
query.sort = sort;
		...
		if (publishFilter) {
			query.filter = { ...(query.filter as Record<string, unknown>), is_published: publishFilter === 'published' ? true : false };
		}

		router.get(animeIndex.url({ query } as RouteQueryOptions), undefined, { preserveState: true, preserveScroll: true });
	}
        ...
        {#if canReadDrafts}
            <ButtonGroup.Root>
                <Button
                    ...
                    class="gap-1"
                    onclick={() => {
 publishFilter = publishFilter === 'published' ? null : 'published'; applyFilters(); 
}}
                >
                    <Eye class="size-4" />
                    Published
                ...
                <Button
                    ...
                    class="gap-1"
                    onclick={() => {
 publishFilter = publishFilter === 'draft' ? null : 'draft'; applyFilters(); 
}}
                >
                    <EyeOff class="size-4" />
                    Draft
