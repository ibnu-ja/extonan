<script lang="ts">
	...
	const footerNavItems = $derived(<NavItem[]>[
		...(!isLoggedIn ? [
			...
			...(canRegister ? [{ title: 'Register', href: registerRoute(), icon: UserPlus }] : []),
		] : []),
	]);
</script>

<Sidebar.Root bind:ref collapsible="icon" variant="inset" class="hidden md:flex" {...restProps}>
	<Sidebar.Header>
		<Sidebar.Menu>
			<Sidebar.MenuItem>
				<Sidebar.MenuButton size="lg" class="hover:bg-transparent hover:text-inherit active:bg-transparent active:text-inherit data-[state=open]:hover:bg-transparent data-[state=open]:hover:text-inherit">
					{#snippet child({ props })}
