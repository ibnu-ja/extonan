    ...
    <div class="ml-auto flex items-center gap-2 md:hidden">
        {#if auth.user}
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" class="w-56 animate-in fade-in slide-in-from-top-2 duration-200">
                    <UserMenuContent user={auth.user}>
                        <DropdownMenuItem asChild>
                            {#snippet children(props)}
                                <Link href={toUrl(dashboard())} class={props.class} onclick={props.onClick}>
        ...
        {:else if canRegister}
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" class="w-40 animate-in fade-in slide-in-from-top-2 duration-200">
                    <DropdownMenuItem asChild>
                        {#snippet children(props)}
                            <Link href={toUrl(login())} class={props.class} onclick={props.onClick}>
                                Log in
                            </Link>
                        {/snippet}
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                        {#snippet children(props)}
                            <Link href={toUrl(registerRoute())} class={props.class} onclick={props.onClick}>
                                Register
                            </Link>
                        {/snippet}
                    </DropdownMenuItem>
