...
- If the user doesn't see a frontend change reflected in the UI, ask them to run `bun run dev`.
...
- This session runs inside the Docker container. Do NOT prefix commands with `vendor/bin/sail`.
- Use `bun`, `php`, `php artisan`, `./vendor/bin/pint` directly.
- NEVER run `bun run build`. Only `bun run lint` for verification.
...
- Run the minimal number of tests needed to ensure code quality and speed. Use `php artisan test --compact` with a specific filename or filter.
...
- If you receive an "Illuminate\Foundation\ViteException: Unable to locate file in Vite manifest" error, run `bun run lint` first.
...
Use caveman ultra skills for communication mode and code review formatting. PERSISTENT across all responses. Do not revert to normal mode mid-chat or across turns. Active until user says "stop caveman" or "normal mode".
